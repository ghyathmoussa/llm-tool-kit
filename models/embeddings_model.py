...
# Argument parsing
parser = argparse.ArgumentParser(description="Train Arabic Embedding Model")
parser.add_argument("--data-path", type=str, required=True, help="Path to the data file (JSONL format)")
parser.add_argument("--batch-size", type=int, default=8, help="Batch size for training")
parser.add_argument("--learning-rate", type=float, default=2e-4, help="Learning rate for optimizer")
...
if __name__ == "__main__":
    """ Load the data """
    texts = get_data_iter(path=args.data_path)
    arabic_texts = [text for text in texts]
    if not arabic_texts:
        print("Error: No texts loaded from the specified data file. Please check the file path and content.")
